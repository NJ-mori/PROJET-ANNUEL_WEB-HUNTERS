...
if ($id <= 0 || !isset($allowed_actions[$action])) {
    logAction(
        "update_captcha_stats: Invalid parameters id=$id action=$action.",
    );
...
try {
    ...
    $stmt = $pdo->prepare(
        "UPDATE captcha_images SET $column = $column + 1 WHERE id = :id",
    );
    $stmt->execute(["id" => $id]);

    if ($stmt->rowCount() === 0) {
        logAction(
            "update_captcha_stats: Image not found id=$id action=$action.",
        );
